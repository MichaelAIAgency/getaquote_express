...
export function StepPropertyType({ value, onChange }: Props) {
  const options: { value: PropertyType; label: string; desc: string; Icon: React.ElementType }[] = [
    { value: 'residential', label: 'Residential', desc: 'Houses, Condos, Apartments', Icon: Home },
    { value: 'commercial', label: 'Commercial', desc: 'Offices, Retail, Warehouses', Icon: Store },
  ];

  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-1">Property Type</h2>
      <p className="text-white/50 text-sm mb-6">Select the type of property we will be painting.</p>
      <div className="grid grid-cols-2 gap-4">
        {options.map(({ value: v, label, desc, Icon }) => (
          <button
            key={v}
            onClick={() => onChange(v)}
            className="relative flex flex-col items-center gap-3 p-6 rounded-2xl transition-all duration-200 text-center cursor-pointer group hover:scale-[1.02]"
            style={
              value === v
                ? {
